...
void io::print_command_menu(const vector<menu_item> &items, const string &header)
{
    ...
    {
        string menu_item = std::to_string(top_items[i]->id) + (i + 1 >= 10 ? ". " : ".  ") + top_items[i]->title;

        int visible_item_len = format::count_visible_chars(menu_item);
        int item_padding_left = UI_LIST_LEFT_PADDING;
        int item_padding_right = width - visible_item_len - UI_LIST_LEFT_PADDING - 1;

        cout << outer_padding << "│" << get_color_code(top_items[i]->clr) << string(item_padding_left, ' ')
             << menu_item << string(item_padding_right, ' ') << get_color_code(CYAN) << " │" << endl;
    }
    // Exit items
    cout << outer_padding << "├" << border << "┤" << endl;
    for (size_t i = 0; i < bottom_items.size(); ++i)
    {
        ...
        string menu_item = std::to_string(id) + (i + 1 >= 10 ? ". " : ".  ") + bottom_items[i]->title;

        int visible_item_len = format::count_visible_chars(menu_item);
        int item_padding_left = id >= 0 ? UI_LIST_LEFT_PADDING : UI_LIST_LEFT_PADDING - 1;
        int item_padding_right = width - visible_item_len - item_padding_left - 1;

        cout << outer_padding << "│" << get_color_code(bottom_items[i]->clr) << string(item_padding_left, ' ')
             << menu_item << string(item_padding_right, ' ') << get_color_code(CYAN) << " │" << endl;
    }
...
void io::print_text_with_header(const string &text, const string &header, const string &link, HEADER_STYLE style, COLOR clr)
{
    ...
    cout << "╭" << border << "╮" << endl;
    cout << "│" << string(header_padding, ' ') << text_color << header << cyan << string(width - header_len - header_padding, ' ') << "│" << endl;
    cout << "├" << border << "┤" << endl;
...
void io::print_table(const string &header, const vector<vector<char>> &table, char operation_symbol, COLOR clr)
{
    print_header(header, BOXED, clr);

    const auto t_clr = get_color_code(clr);
    const auto e_clr = get_color_code(BASE);
    const auto yellow = get_color_code(YELLOW);
    ...
    cout << outer_padding << t_clr << "╭" << cell;
    for (int i = 0; i < n; i++)
        cout << "┬" << cell;
    cout << "╮\n";

    cout << outer_padding << "│ " << get_color_code(PURPLE) << operation_symbol << t_clr << " │";
    for (int i = 0; i < n; i++)
        cout << " " << yellow << elems[i] << t_clr << " │";
    cout << "\n";

    cout << outer_padding << "├" << cell;
    for (int i = 0; i < n; i++)
        cout << "┼" << cell;
    cout << "┤\n";
    ...
    for (int i = 0; i < n; i++)
    {
        cout << outer_padding << "│ " << yellow << elems[i] << t_clr << " │";
        for (int j = 0; j < n; j++)
            cout << " " << e_clr << table[i][j] << t_clr << " │";
        cout << "\n";

        if (i != n - 1) // промежуточный разделитель
        {
            cout << outer_padding << "├" << cell;
            for (int k = 0; k < n; k++)
                cout << "┼" << cell;
...
void io::print_operation_result(const string &result, COLOR clr)
{
    int len = format::count_visible_chars(result);
    int header_len = format::count_visible_chars("Результат операции");
    int width = max(UI_OPERATION_RESULT_LENGTH, max(len, header_len) + 12);
    ...
    int header_padding_left = (width - header_len) / 2;
    int header_padding_right = width - header_len - header_padding_left;

    string border = get_border(width, "─");

    cout << outer_padding << get_color_code(CYAN);
    cout << "╭" << border << "╮" << endl;
    cout << outer_padding << "│" << string(header_padding_left, ' ') << get_color_code(clr)
         << "Результат операции" << get_color_code(CYAN) << string(header_padding_right, ' ') << "│" << endl;
    cout << outer_padding << "├" << border << "┤" << endl;
    cout << outer_padding << "│" << string(padding_left, ' ') << get_color_code(clr) << result
         << get_color_code(CYAN) << string(padding_right, ' ') << "│" << endl;
        ...
        try
        {
            ...
            else
            {
                string error_message =
                    "Число должно быть от " + std::to_string(min_max_id.first) + " до " + std::to_string(min_max_id.second);
