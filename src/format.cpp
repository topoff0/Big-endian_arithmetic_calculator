...
vector<string> format::split_text(const string &text, int max_line_length, ALIGN align)
{
    vector<string> lines;
    ...
    for (size_t p = 0; p < paragraphs.size(); ++p)
    {
        ...
        while (byte_pos < len)
        {
            ...
            if (cur + w + space > max_line_length)
            {
                ...
                {
                    // Применяем выбранное выравнивание
                    if (align == ALIGN_CENTER)
                        lines.push_back(align_center(current_line, max_line_length));
                    else if (align == ALIGN_JUSTIFY)
                        lines.push_back(align_justify(current_line, max_line_length));
                    else
        ...
        // Последняя строка абзаца — НИКОГДА не justify
        if (!current_line.empty())
        {
            if (align == ALIGN_CENTER)
                lines.push_back(align_center(current_line, max_line_length));
