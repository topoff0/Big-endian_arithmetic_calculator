...
public:
    ...
    static void print_command_menu(const vector<menu_item> &items, const string &header);
    ...
    static void print_table(const string &header, const vector<vector<char>> &table, char operation_symbol, COLOR clr = CYAN);
    static void print_operation_result(const string &result, COLOR clr = BASE);

    // === Error ===
    static void print_error(const string &message, COLOR clr = RED);

    // === Input ===
    static int read_number(const pair<int, int> &min_max_id, const string &header);
